...
# Print header
echo -e "${BLUE}${BOLD}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}${BOLD}║         Ivan Linting Routine           ║${NC}"
echo -e "${BLUE}${BOLD}╚════════════════════════════════════════╝${NC}"
echo ""

# Check if we're in the right directory
if [ ! -f "ivan.py" ]; then
    echo -e "${RED}Error: ivan.py not found. Please run from Ivan/ directory.${NC}"
    exit 1
fi
...
# Count Python files (exclude venv and other directories)
PY_COUNT=$(find "$TARGET" -name "*.py" -not -path "*/venv/*" -not -path "*/.venv/*" -not -path "*/deprecated/*" -not -path "*/hashicorp_web_docs/*" | wc -l)
...
if command_exists mypy; then
    ...
    echo -e "${YELLOW}Run manually: mypy ivan.py tools.py hashicorp_doc_search.py${NC}"
    echo ""
fi
